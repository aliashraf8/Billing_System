...
//CLEAR SCREEN FUNCTION COMPATIBLE WITH DIFFRENT OS.
void CLEAR_SCREEN()
{
    #ifdef _WIN32
        system("cls");
...
int main()
{
    ...
    GET_INT(&LENGTH);  // USING FUNCTION AND PASSING BY REFERENCE.
...
void PRINT_BILL_TERMINAL(item ar[], int x, float S_T, float V, float V_A, float T_A_V)
    {
        ...
        /*-------------------------------------------------
         ...
         //PRINITN ITEMS AND DETAILS IN ROWS FOR EACH ITEM.
        for(int j = 0; j < x; j++)
        {
